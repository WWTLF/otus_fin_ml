...
- **downloader [Container: python]**  
  Микросервис, загружающий данные с Yahoo Finance и записывающий их в базу данных. Сервис реализован на основе Docker контейнера, cм. Dockerfile.2

- **diction maker [Container: python]**  
  Сервис, анализирующий данные из базы и генерирующий торговые рекомендации. Сервис реализован на основе Docker того же контейнера, но запускается с другими параметрами, cм. Dockerfile.2
...
- **Триггеры [yandex cloud]**  
  Бессерверные контейнеры запускаются тригреами раз в сутки, с перерывом в один час. 
